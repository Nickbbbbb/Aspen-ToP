...
3. 保证 `processEdges.source.port` 和 `processEdges.target.port` 都存在于对应节点的 `ports` 中。

修改组分或物性方法：

1. 修改 `assets_builder.py`。
2. 组分逻辑不写进 `graph_builder.py`。

修改项目元信息：

1. 修改 `project_builder.py`。
2. 项目名、归档版本、固定时间等字段集中在该文件维护。
